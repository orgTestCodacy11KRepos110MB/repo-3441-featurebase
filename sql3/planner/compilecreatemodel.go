...
func (p *ExecutionPlanner) compileCreateModelStatement(stmt *parser.CreateModelStatement) (types.PlanOperator, error) {
	...
	query := NewPlanOpQuery(p, NewPlanOpCreateModel(p, model, selOp), p.sql)
	return query, nil
}

func (p *ExecutionPlanner) analyzeCreateModelStatement(stmt *parser.CreateModelStatement) error {
	// iterate the options
	for _, opt := range stmt.Options {
		...
		e, err := p.analyzeModelOptionExpr(optName, opt.OptionExpr, stmt)
		if err != nil {
			return err
		}
		opt.OptionExpr = e
	...
	// analyze the select
	_, err := p.analyzeSelectStatement(stmt.ModelQuery)
	if err != nil {
		return err
	}
	return nil
...
func (p *ExecutionPlanner) analyzeModelOptionExpr(optName string, expr parser.Expr, scope parser.Statement) (parser.Expr, error) {
	if expr == nil {
		return nil, nil
	}

	e, err := p.analyzeExpression(expr, scope)
	if err != nil {
		return nil, err
	}
